...
export default function UserManagement() {
  ...
  // Filter users based on status filter, position filter, access filter, and search query
  useEffect(() => {
    ...
    if (searchQuery.trim() !== "") {
      ...
      filtered = filtered.filter(
        (user) =>
          ...
          (user.access && user.access.toLowerCase().includes(query)),
      );
    }
    ...
    <View style={{ flex: 1, padding: 16 }}>
      ...
      >
        ...
        <Picker
          ...
          onValueChange={(itemValue) => setPositionFilter(itemValue)}
          style={[
            styles.filterPicker,
          ...
          mode="dropdown"
        >
          <Picker.Item label="Position" value="All" />
