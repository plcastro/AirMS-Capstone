...
export default function TaskChecklist({ 
  ...
  useEffect(() => {
    if (task?.checklistItems) {
      ...
      } else if (task.status === "Completed" || task.status === "Turned in" || task.status === "Approved") {
        setChecklistState(task.checklistItems.map(() => true));
      } else {
        setChecklistState(task.checklistItems.map(() => false));
      }
      
      if (task.status === "Completed" || task.status === "Turned in" || task.status === "Approved") {
        setIsStarted(true);
      } else {
  ...
  const toggleItem = (index) => {
    ...
    if (task.status === "Completed" || task.status === "Turned in" || task.status === "Approved") return;
  ...
  const isCompleted = task.status === "Completed" || task.status === "Turned in" || task.status === "Approved";
  ...
  const allCheckboxesChecked = checklistState.every(item => item === true) && checklistState.length > 0;
  ...
  return (
    <>
          ...
          >
            ...
            {isHeadView && isReturned && (
              <View style={{
                ...
                <Text style={{ fontSize: 16, fontWeight: "600", color: "#c62828", marginBottom: 4 }}>
                  Returned for Rework
                </Text>
                {task.returnComments && (
            ...
            {isReturned && !isHeadView && (
              <View style={{
                ...
                <Text style={{ fontSize: 16, fontWeight: "600", color: "#c62828", marginBottom: 8 }}>
                  Remarks
                </Text>
                <Text style={{ fontSize: 14, color: "#b71c1c", marginBottom: 12 }}>
                  {task.returnComments || "Finding details are incomplete. Please update findings."}
                </Text>
                <Text style={{ fontSize: 12, color: "#e57373" }}>
                  Returned on {formatReturnedDateTime(task.returnedDate || new Date())}
                </Text>
              </View>
            )}
            ...
            {isCompleted && (
              <View style={{
                ...
                <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 12 }}>
                  ...
                  <Text style={{ fontSize: 16, color: "#2e7d32", fontWeight: "600" }}>
                    Task Completed
                  </Text>
                </View>
                ...
                <View style={{
                  ...
                  {isApproved ? (
                    <View>
                      <Text style={{ fontSize: 16, color: "#2e7d32", fontWeight: "600", marginBottom: 2 }}>
                        ✓ Approved
                      </Text>
                    ...
                    <View>
                      <Text style={{ fontSize: 16, color: "#f57c00", fontWeight: "600", marginBottom: 2 }}>
                        ⏳ Not Yet Approved
                      </Text>
                      <Text style={{ fontSize: 13, color: "#666" }}>
            ...
            <ScrollView style={{ marginBottom: 20 }} showsVerticalScrollIndicator={false}>
              ...
              {!isHeadView && isStarted && (
                <>
                  <Text style={{ fontSize: 18, fontWeight: "600", marginTop: 20, marginBottom: 12 }}>
                    Findings
                  </Text>
                  <View style={{
                    borderWidth: 1,
                ...
                <>
                  ...
                  {!isStarted ? (
                    ...
                    <Button
                      ...
                      onPress={allCheckboxesChecked ? () => handleTurnIn() : handleSave}
                      buttonStyle={[
                        styles.primaryAlertBtn,
                        { width: 100 },
                      ]}
      ...
      <ReviewTask
        ...
        onConfirm={reviewMode === "return" ? handleReturnConfirm : handleApproveConfirm}
        mode={reviewMode}
      />
    </>
  );
