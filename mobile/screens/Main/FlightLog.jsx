...
const fetchFlightLogs = async () => {
  try {
    ...
    console.log('Fetching from:', `${API_BASE}/api/flightlogs?${params.toString()}`);
    ...
    Alert.alert("Error", "Failed to connect to server. Please check your network.");
  } finally {
    setLoading(false);
    setRefreshing(false);
  }
...
const searchFlightLogs = async (query) => {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/api/flightlogs/search?q=${encodeURIComponent(query)}&limit=100`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
...
const handleSaveNewEntry = async (newEntry) => {
  ...
  const handleSaveEdit = async (updatedLog) => {
    try {
      const response = await fetch(`${API_BASE}/api/flightlogs/${updatedLog._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const aircraftOptions = ["all", ...new Set(flightLogs.map((log) => log.rpc).filter(Boolean))];
  ...
  const filteredLogs = flightLogs.filter((log) => {
    ...
    const matchesAircraft = selectedAircraft === "" || 
      ...
      <View style={{ flex: 1, paddingHorizontal: 16, paddingTop: 8 }}>
        ...
        <View style={{ flexDirection: "row", marginBottom: 12, gap: 12 }}>
          <View style={{
            ...
            <MaterialCommunityIcons name="magnify" size={22} color={COLORS.grayDark} />
            <TextInput
          ...
          >
            ...
            <Text style={{ color: COLORS.white, fontSize: 15, fontWeight: "600", marginLeft: 6 }}>
              New Entry
            </Text>
          </TouchableOpacity>
        </View>
        ...
        {loading && !refreshing && (
          <View style={{ flex: 1, justifyContent: "center", alignItems: "center", paddingTop: 50 }}>
            ...
            <Text style={{ marginTop: 10, color: COLORS.grayDark }}>Loading flight logs...</Text>
          </View>
        )}
        ...
        {!loading && (
          ...
          >
            {filteredLogs.length === 0 ? (
              <View style={{ flex: 1, justifyContent: "center", alignItems: "center", paddingTop: 50 }}>
                <MaterialCommunityIcons name="file-document-outline" size={60} color={COLORS.grayMedium} />
                <Text style={{ marginTop: 10, fontSize: 16, color: COLORS.grayDark, textAlign: "center" }}>
                ...
                >
                  <Text style={{ color: COLORS.white, fontWeight: "600" }}>Create New Entry</Text>
                </TouchableOpacity>
              </View>
            ) : (
              <FlightLogCards 
