...
def login():
    try:
        ...
        print(f"[DEMO LOGIN OTP] email={user.get('email')} otp={otp}")
        return jsonify(
            {
...
def resend_login_otp():
    ...
    print(f"[DEMO LOGIN OTP RESEND] email={user.get('email')} otp={otp}")
...
def request_password_reset():
    ...
    print(f"[DEMO PASSWORD RESET OTP] email={email} otp={otp}")
