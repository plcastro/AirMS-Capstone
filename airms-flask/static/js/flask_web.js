(() => {
  ...
  const verifyCurrentUserPin = async (pin) => {
    ...
    if (!/^\d{6}$/.test(cleanPin)) throw new Error("Enter your 6-digit PIN to confirm.");
    const user = getUser();
    const userId = user?.id || user?._id;
    if (!userId) throw new Error("Your user ID is missing. Please sign in again.");
  ...
  const initFlightLogPage = () => {
    ...
    const emptyComponentSection = () => Object.fromEntries(componentFields.map(([key]) => [key, ""]));
    ...
    const emptyFuelItem = () => ({ date: "", contCheck: "", mainRemG: "", mainAdd: "", mainTotal: "", refuelerName: "", fuelType: "", signature: "" });
    ...
    const emptyWorkItem = () => ({ id: `work-${Date.now()}-${Math.random()}`, selectedWorkTypes: [], date: "", aircraft: "", workDone: "", name: "", certificateNumber: "", signature: "" });
    ...
    const normalizeLegsForForm = (legs = []) => {
      ...
      return rows.map((leg) => ({
        ...emptyLeg(),
        ...leg,
        stations: Array.isArray(leg?.stations) && leg.stations.length ? leg.stations.map((station) => ({ from: station?.from || "", to: station?.to || "" })) : [{ from: "", to: "" }],
    ...
    const syncServicingRows = () => {
      fuelServicing = formLegs.map((_, index) => ({ ...emptyFuelItem(), ...(fuelServicing[index] || {}) }));
      oilServicing = formLegs.map((_, index) => ({ ...emptyOilItem(), ...(oilServicing[index] || {}) }));
    };
    ...
    const userDisplayName = () => [getUser()?.firstName, getUser()?.lastName].filter(Boolean).join(" ") || getUser()?.username || "AirMS User";
    const buildToDateData = (log) => {
      ...
      const sum = (key) => (Number(broughtForward[key]) || 0) + (Number(thisFlight[key]) || 0);
      return Object.keys(existing).length ? existing : {
    ...
    const exportFlightLog = (log) => {
      ...
      if (!win) return;
      const destinations = (log.legs || []).flatMap((leg) => leg.stations || []).map((station) => `${station.from || ""} - ${station.to || ""}`).filter((value) => value.trim()).join(", ") || "N/A";
      const toDate = buildToDateData(log);
      win.document.write(`<!doctype html><html><head><title>Flight Log ${escapeHtml(log.rpc || getId(log))}</title><style>
        ...
        <table><tbody>${Object.entries(toDate).map(([key, value]) => `<tr><th>${escapeHtml(labelize(key))}</th><td>${escapeHtml(value || "0")}</td></tr>`).join("")}</tbody></table>
        <h2>Signatures</h2>
        <table><tbody>
    ...
    const renderLegs = () => {
      if (!legsRoot) return;
      const canEdit = !isOic();
      legsRoot.innerHTML = formLegs.map((leg, legIndex) => `
    ...
    const renderComponents = () => {
      ...
      calculateToDateData();
      componentsRoot.innerHTML = componentSections.map(([sectionKey, title, locked]) => `
        <section class="flight-component-section">
          <h5>${escapeHtml(title)}</h5>
          ${componentFields.map(([key, label, type]) => `
            <label class="field compact">
              <span>${escapeHtml(label)}</span>
    ...
    const serviceField = (group, legIndex, key, label, value, type = "text") => `
      <label class="field compact">
        <span>${escapeHtml(label)}</span>
    ...
    const renderFuelOil = () => {
      syncServicingRows();
      if (fuelRoot) {
        fuelRoot.innerHTML = fuelServicing.map((fuel, index) => `
      ...
      if (oilRoot) {
        oilRoot.innerHTML = oilServicing.map((oil, index) => `
          <section class="flight-leg-card">
            <div class="section-head"><h5>Leg ${index + 1}</h5></div>
            <div class="flight-leg-fields">
    ...
    const renderWorkItems = () => {
      if (!workRoot) return;
      workRoot.innerHTML = workItems.length ? workItems.map((item, index) => `
      ...
      `).join("") : `<p class="muted">No work items yet.</p>`;
    };
    const openForm = (log = null) => {
      editing = log;
      form.reset();
      $$("input,select,textarea", form).forEach((node) => { node.disabled = false; });
      $("[data-flight-dialog-title]").textContent = log ? "Edit Flight Log" : "New Flight Log";
      ["rpc", "aircraftType", "date", "controlNo", "route", "pilot", "totalFlightTime", "status", "remarks", "sling"].forEach((key) => {
        if (!form[key]) return;
      ...
      fuelServicing = Array.isArray(log?.fuelServicing) ? log.fuelServicing : [];
    ...
    const collect = () => {
      ...
      body.legs = formLegs.map((leg) => ({
        ...leg,
        stations: (leg.stations || []).filter((station) => station.from || station.to),
      }));
      calculateToDateData();
    ...
    const runWorkflow = async (event) => {
      ...
      try {
        if (action === "notify") {
          ...
          await apiFetch(`/api/parts-monitoring/${encodeURIComponent(aircraft)}/update-totals`, {
            method: "PUT",
            body: JSON.stringify({
          ...
          await apiFetch(`/api/flightlogs/${encodeURIComponent(id)}/complete`, { method: "PUT", body: JSON.stringify({}) });
        } else {
          const signature = workflowForm.signature?.value || "";
          await verifyCurrentUserPin(workflowForm.pin.value);
          await apiFetch(`/api/flightlogs/${encodeURIComponent(id)}/${action}`, { method: "PUT", body: JSON.stringify({ name: userDisplayName(), signature }) });
        }
    ...
    legsRoot?.addEventListener("click", (event) => {
      ...
      const removeStation = event.target.closest("[data-flight-remove-station]");
      ...
      if (addStation) {
        ...
        formLegs[legIndex].stations = [...stations, { from: last.to || "", to: "" }];
        renderLegs();
      }
      if (removeStation) {
        const legIndex = Number(removeStation.dataset.legIndex);
        const stationIndex = Number(removeStation.dataset.flightRemoveStation);
        formLegs[legIndex].stations = (formLegs[legIndex].stations || []).filter((_, index) => index !== stationIndex);
        renderLegs();
      }
      if (removeLeg) {
        formLegs = formLegs.filter((_, index) => index !== Number(removeLeg.dataset.flightRemoveLeg));
    ...
    legsRoot?.addEventListener("input", (event) => {
      ...
      if (stationInput) {
        ...
        if (key === "to" && formLegs[legIndex].stations[stationIndex + 1]) formLegs[legIndex].stations[stationIndex + 1].from = stationInput.value;
      }
      if (legInput) {
        formLegs[Number(legInput.dataset.legIndex)][legInput.dataset.flightLegKey] = legInput.value;
      }
    });
    ...
    workRoot?.addEventListener("click", (event) => {
      ...
      if (!button) return;
      workItems = workItems.filter((_, index) => index !== Number(button.dataset.flightRemoveWork));
      renderWorkItems();
    });
  ...
  const initTaskAssignmentPage = () => {
    ...
    const load = async () => {
      ...
      try {
        const [taskPayload, userPayload, aircraftPayload, schedulePayload] = await Promise.all([
    ...
    const openForm = (task = null) => {
      ...
      const aircraftOptions = [...new Set([...aircraftList, ...tasks.map((item) => item.aircraft)].filter(Boolean))];
      $("[data-task-aircraft-list]").innerHTML = aircraftOptions.map((item) => `<option value="${escapeHtml(item)}"></option>`).join("");
      $("[data-task-schedules]").innerHTML = `<option value="">Custom Task</option>${schedules.map((schedule) => `<option value="${escapeHtml(getId(schedule))}">${escapeHtml(schedule.name || schedule.inspectionName || schedule.title || getId(schedule))}</option>`).join("")}`;
      $("[data-task-form-title]").textContent = task ? "Edit Task Assignment" : "Create Task Assignment";
      ["title", "aircraft", "assignedTo", "priority", "maintenanceType", "status", "inspectionSchedule"].forEach((key) => { if (taskForm[key]) taskForm[key].value = task?.[key] || task?.inspectionId || (key === "priority" ? "Normal" : key === "status" ? "Pending" : ""); });
    ...
    const collectTask = () => {
      ...
      body.inspectionId = body.inspectionSchedule || editingTask?.inspectionId || "";
    ...
    $("[data-task-schedules]")?.addEventListener("change", async (event) => {
      ...
      try {
        const schedule = schedules.find((item) => getId(item) === scheduleId) || {};
        const templatePayload = await apiFetch(`/api/inspections/tasks?inspectionId=${encodeURIComponent(scheduleId)}`).catch(() => []);
        const templateRows = normalizeRows(templatePayload);
        taskForm.title.value = schedule.name || schedule.inspectionName || schedule.title || taskForm.title.value || "Inspection Task";
        taskForm.maintenanceType.value = "Inspection";
        if (!taskForm.aircraft.value && schedule.aircraft) taskForm.aircraft.value = schedule.aircraft;
        if (templateRows.length) {
          taskForm.checklistText.value = templateRows.map((item) => item.taskName || item.description || item.name || item.title).filter(Boolean).join("\n");
        }
      } catch (error) {
        $("[data-task-form-status]").textContent = error.message;
      }
  ...
  const initMaintenanceLogPage = () => {
    ...
    const exportMaintenanceReport = async () => {
      ...
      try {
        aircraftData = await apiFetch(`/api/parts-monitoring/${encodeURIComponent(selectedWO.aircraft)}`).then((payload) => payload.data || payload);
      } catch {
        aircraftData = null;
      }
      const win = window.open("", "_blank", "noopener,noreferrer");
      if (!win) return;
      const workRows = (Array.isArray(selectedWO.workDetails) && selectedWO.workDetails.length ? selectedWO.workDetails : [{ description: selectedWO.correctiveActionDone || selectedWO.defects || "" }])
        .map((item, index) => `<tr><td>${index + 1}</td><td>${escapeHtml(item.description || item || "")}</td></tr>`).join("");
      const displayDate = (value) => {
        if (!value) return "N/A";
        const parsed = new Date(value);
        return Number.isNaN(parsed.getTime()) ? String(value) : parsed.toLocaleDateString("en-US");
      };
      win.document.write(`<!doctype html><html><head><title>Maintenance Log ${escapeHtml(selectedWO.sourceTaskId || selectedWO.id || "")}</title><style>
    ...
    exportButton?.addEventListener("click", () => exportMaintenanceReport().catch((error) => alert(error.message)));
  ...
  const initPartsLifespanPage = () => {
    ...
    let parts = [];
    let referenceData = {};
    const editablePartColumns = new Set(["dateCW", "hoursCW", "timeSinceInstall", "totalTimeSinceNew"]);
    const exportColumns = [
      ["componentName", "DUE Indicates Items Due Within 30 Hours, 30 Days, or 30 Cycles/Landings"],
    ...
    const computeTimeRemaining = (part) => {
      ...
      const limit = toNumber(part.hourLimit2 ?? part.hourLimit1 ?? part.ttCycleDue);
      const hoursCW = toNumber(part.hoursCW);
      const acftTT = toNumber(referenceData.acftTT);
      if (limit !== null && hoursCW !== null && acftTT !== null) return Number((limit - (acftTT - hoursCW)).toFixed(2));
      return stored ?? "";
    };
    const remainingDays = (part) => {
      ...
      if (part.dateDue) {
        ...
        const diff = Math.ceil((new Date(part.dateDue).getTime() - today.getTime()) / 86400000);
    ...
    const dueText = (part) => {
      ...
      const existingDue = String(part.due || "").trim().toUpperCase();
      if ((Number.isFinite(days) && days <= 0) || (Number.isFinite(hours) && hours <= 0) || existingDue === "DUE") return "DUE";
    ...
    const renderTable = () => {
      ...
      tableBody.innerHTML = rows.map((rawPart) => {
        const part = computedPart(rawPart);
        const due = part.due;
        const editableCell = (key, type = "text") => editablePartColumns.has(key)
    ...
    $("[data-parts-save]")?.addEventListener("click", async () => {
      ...
      try {
        currentDoc = await apiFetch("/api/parts-monitoring/save", { method: "POST", body: JSON.stringify({ ...(currentDoc || {}), aircraft: selectedAircraft, referenceData: collectRefs(), parts: parts.map(computedPart), confirmAction: true }) }).then((payload) => payload.data || payload);
    ...
    $("[data-parts-export]")?.addEventListener("click", () => {
      ...
      const rows = [
        ...
        [`Date: ${referenceData.today || ""}`, `Acft. TT: ${referenceData.acftTT || ""}`, `N1: ${referenceData.n1Cycles || ""}`, `N2: ${referenceData.n2Cycles || ""}`, `Landings: ${referenceData.landings || ""}`],
        [],
        exportColumns.map(([, label]) => label),
        ...filteredParts().map(computedPart).map((part) => exportColumns.map(([key]) => part[key] ?? "")),
      ];
    ...
    tableBody?.addEventListener("input", (event) => {
      ...
      if (!input) return;
      const row = parts.find((part) => String(part._id) === String(input.dataset.partEdit));
      if (!row) return;
      row[input.dataset.partKey] = input.type === "number" ? Number(input.value || 0) : input.value;
      renderTable();
      const nextInput = $(`[data-part-edit="${CSS.escape(input.dataset.partEdit)}"][data-part-key="${CSS.escape(input.dataset.partKey)}"]`);
      nextInput?.focus();
    });
    $$("[data-ref]").forEach((input) => input.addEventListener("input", () => {
      collectRefs();
      renderTable();
    }));
  ...
  const initMaintenanceTrackingPage = () => {
    ...
    const riskCounts = () => filteredInsights().reduce((acc, item) => {
      const risk = item.riskLevel || item.priorityLevel || "Low";
      acc.total += 1;
      acc[risk.toLowerCase()] = (acc[risk.toLowerCase()] || 0) + 1;
      return acc;
    }, { total: 0, critical: 0, high: 0, medium: 0, low: 0 });
    const filteredInsights = () => selectedAircraft() === "all" ? insights : insights.filter((item) => item.aircraft === selectedAircraft());
    const filteredRemaining = () => selectedAircraft() === "all" ? remaining : remaining.filter((item) => item.aircraft === selectedAircraft());
    const scheduledTasks = () => filteredInsights().flatMap((item) => (item.scheduledTasks || []).map((task) => ({ ...task, aircraft: item.aircraft })));
    const renderAircraftOptions = () => {
      const aircraft = [...new Set([...insights.map((item) => item.aircraft), ...remaining.map((item) => item.aircraft)].filter(Boolean))].sort();
    ...
    const renderHealth = () => {
      ...
      const message = health?.cooldown?.active
        ...
        : health?.message || meta?.llmLastResult?.message || "Rule-based maintenance insights are active.";
    ...
    const insightSummary = (item) => item.managerSummary || item.shortFinding || item.issueTitle || "Maintenance review required";
    const referenceText = (item) => [item.procedureReference, item.procedureTitle, ...(item.manualReferences || [])].filter(Boolean).join(" | ") || item.component || "-";
    const renderTracking = () => {
      ...
      renderHealth();
      const highest = filteredInsights().find((item) => ["Critical", "High"].includes(item.riskLevel)) || filteredInsights()[0];
      ...
      $("[data-tracking-insights]").innerHTML = [
        highest ? `${highest.aircraft}: ${insightSummary(highest)} (${highest.riskLevel || "Low"}).` : "No active maintenance findings are currently detected.",
      ...
      $("[data-tracking-findings]").innerHTML = filteredInsights().map((item, index) => `<tr><td>${escapeHtml(item.aircraft || "-")}</td><td>${renderStatus(item.riskLevel || "Low")}</td><td><strong>${escapeHtml(item.issueTitle || "-")}</strong><p class="muted">${escapeHtml(insightSummary(item))}</p></td><td>${escapeHtml(item.recommendedAction || (item.recommendedActions || []).join(" | ") || "-")}</td><td>${escapeHtml(referenceText(item))}</td><td><button type="button" class="btn small ghost" data-rectify="${index}">Rectify</button></td></tr>`).join("") || `<tr><td colspan="6" class="muted">No findings.</td></tr>`;
      $("[data-tracking-tasks]").innerHTML = scheduledTasks().map((task) => `<tr><td>${escapeHtml(task.aircraft || "-")}</td><td>${escapeHtml(task.title || "-")}</td><td>${escapeHtml(task.assignedToName || "Unassigned")}</td><td>${escapeHtml(task.startDateTime || "-")}</td><td>${escapeHtml(task.endDateTime || task.dueDate || "-")}</td><td>${escapeHtml(task.priority || "Normal")}</td><td>${renderStatus(task.status || "-")}</td></tr>`).join("") || `<tr><td colspan="7" class="muted">No scheduled tasks.</td></tr>`;
    ...
    const load = async (includeAiSummary = false) => {
      $("[data-tracking-findings]").innerHTML = `<tr><td colspan="6" class="muted">Loading AI maintenance insights...</td></tr>`;
      const url = includeAiSummary ? "/api/ai-insights/maintenance-tracking?includeLLMSummary=1&llmLimit=0" : "/api/ai-insights/maintenance-tracking";
      const [insightPayload, healthPayload, remainingPayload] = await Promise.all([apiFetch(url), apiFetch("/api/ai-insights/health").catch(() => null), apiFetch("/api/parts-monitoring/inspection-remaining-hours")]);
    ...
    regenerateButton?.addEventListener("click", () => {
      if (health?.configured === false) return alert(health.message || "OpenAI is not configured on the server.");
      if (health?.cooldown?.active) return alert(health.cooldown.message || "AI cooldown is active.");
      if (confirm("Regenerate maintenance summaries now?")) load(true).catch((error) => alert(error.message));
    });
    $("[data-tracking-findings]")?.addEventListener("click", async (event) => {
      ...
      if (!item || !confirm(`Mark this finding as rectified for ${item.aircraft || "this aircraft"}?`)) return;
  ...
  const initMaintenanceReportsPage = () => {
    ...
    const knownBase = (value) => !["", "UNKNOWN", "N/A", "NA", "UNASSIGNED"].includes(String(value || "").trim().toUpperCase());
    const firstKnownBase = (...values) => {
      const match = values.find(knownBase);
      return match ? String(match).trim().toUpperCase() : "";
    };
    const countBy = (records, getKey) => records.reduce((acc, record) => {
      const key = getKey(record) || "Unknown";
      acc[key] = (acc[key] || 0) + 1;
      return acc;
    }, {});
    const toRows = (counts) => Object.entries(counts).map(([label, value]) => ({ label, value })).sort((a, b) => b.value - a.value);
    const recordDate = (record = {}) => record.date || record.dateRequested || record.dateAdded || record.createdAt || record.updatedAt;
    ...
    const isCompletedTask = (task = {}) => {
      ...
      return ["completed", "turned in", "approved"].includes(status) || task.isApproved === true || Boolean(task.completedAt);
    };
    ...
    const textHasDamage = (task = {}) => [task.status, task.title, task.findings, task.defects, task.maintenanceType, task.summary?.remarks].filter(Boolean).join(" ").toLowerCase().match(/damage|damaged|defect|crack|fault|issue/);
    const textHasRepair = (task = {}) => isCompletedTask(task) || [task.status, task.title, task.findings, task.defects, task.maintenanceType, task.summary?.remarks].filter(Boolean).join(" ").toLowerCase().match(/repair|repaired|rectified|fixed|resolved/);
    const aircraftBaseLookup = () => state.aircraftBases.reduce((lookup, aircraft) => {
      const tail = String(aircraft.tailNum || aircraft.aircraft || "").trim().toUpperCase();
      const base = String(aircraft.base || "").trim().toUpperCase();
      if (tail && knownBase(base)) lookup[tail] = base;
      return lookup;
    }, {});
    const taskBase = (task, lookup) => firstKnownBase(task.base, task.locationBase, task.assignedBase, task.stationBase, lookup[String(task.aircraft || "").trim().toUpperCase()]) || "UNKNOWN";

    const baseSummary = () => {
      const analyticsRows = (state.baseAnalytics?.byBase || []).filter((row) => knownBase(row.base));
      if (analyticsRows.length) {
        const damageRows = analyticsRows.map((row) => ({ label: row.base, value: row.damagedCount || 0 }));
        const repairRows = analyticsRows.map((row) => ({ label: row.base, value: row.repairedCount || 0 }));
        return {
          damageRows,
          repairRows,
          topDamaged: damageRows.sort((a, b) => b.value - a.value)[0] || { label: "N/A", value: 0 },
          topRepaired: repairRows.sort((a, b) => b.value - a.value)[0] || { label: "N/A", value: 0 },
        };
      }
      const lookup = aircraftBaseLookup();
      const damageRows = toRows(countBy(state.tasks.filter(textHasDamage), (task) => taskBase(task, lookup)));
      const repairRows = toRows(countBy(state.tasks.filter(textHasRepair), (task) => taskBase(task, lookup)));
      return {
        damageRows,
        repairRows,
    ...
    const stats = () => ({
      ...
      dueSoon: state.tasks.filter((task) => taskCategory(task) === "dueSoon").length,
      overdue: state.tasks.filter((task) => taskCategory(task) === "overdue").length,
      modules: 9,
    });
    ...
    const renderMiniChart = (root, rows) => {
      ...
      `).join("") : `<p class="muted">No base data available.</p>`;
    };

    const taskRows = (category) => state.tasks
      .filter((task) => taskCategory(task) === category)
      .map((task, index) => ({
        key: getId(task) || `${task.title || "task"}-${index}`,
        aircraft: task.aircraft || "N/A",
        task: task.title || task.task || task.summary?.category || "Untitled task",
        mechanic: task.assignedToName || task.assignedMechanic || task.assignedTo || "Unassigned",
        type: task.maintenanceType || "N/A",
        dueDate: dateLabel(taskDueDate(task)),
        completed: dateLabel(task.approvedAt || task.completedAt || task.dateRectified || task.updatedAt),
    ...
    const renderTable = () => {
      ...
      const views = {
        dueSoon: { title: "Due Soon Tasks", columns: ["aircraft", "task", "mechanic", "type", "dueDate", "priority", "status", "notes"], rows: taskRows("dueSoon") },
        overdue: { title: "Overdue Tasks", columns: ["aircraft", "task", "mechanic", "type", "dueDate", "priority", "status", "notes"], rows: taskRows("overdue") },
        completed: { title: "Completed Tasks", columns: ["aircraft", "task", "mechanic", "type", "completed", "priority", "status", "notes"], rows: taskRows("completed") },
        baseDamage: { title: "Base Damage Reports", columns: ["label", "value"], rows: summary.damageRows },
        baseRepair: { title: "Base Repaired Aircraft", columns: ["label", "value"], rows: summary.repairRows },
      };
      ...
      tbody.innerHTML = view.rows.length
        ? view.rows.map((row) => `<tr>${view.columns.map((column) => {
    ...
    const reportModules = () => {
      const taskStatusRows = toRows(countBy(state.tasks, (task) => normalizeStatus(task.status)));
      const partStatusRows = toRows(countBy(state.parts, (part) => part.priority || part.due || part.status || "Tracked"));
      const flightStatusRows = toRows(countBy(state.flightLogs, (log) => normalizeStatus(log.status)));
      const preRows = toRows(countBy(state.preInspections, (row) => normalizeStatus(row.status)));
      const postRows = toRows(countBy(state.postInspections, (row) => normalizeStatus(row.status)));
      const reqRows = toRows(countBy(state.requisitions, (row) => normalizeStatus(row.status)));
      return [
        { key: "general", category: "Performance", title: "General Reports", rows: [["Tasks", state.tasks.length], ["Flight Logs", state.flightLogs.length], ["Pre Inspections", state.preInspections.length], ["Post Inspections", state.postInspections.length], ["Parts Requisitions", state.requisitions.length]] },
        { key: "performance", category: "Performance", title: "Performance Overview", rows: taskStatusRows.map((row) => [row.label, row.value]) },
        { key: "history", category: "Performance", title: "Maintenance History", rows: state.tasks.slice(0, 8).map((task) => [dateLabel(recordDate(task)), task.title || task.task || "Untitled task"]) },
        { key: "summary", category: "Performance", title: "Maintenance Insights", rows: [["Completed Tasks", stats().completed], ["Due Soon", stats().dueSoon], ["Overdue", stats().overdue]] },
        { key: "component", category: "Inventory", title: "Component Analysis", rows: partStatusRows.map((row) => [row.label, row.value]) },
        { key: "flight-log", category: "Logbook", title: "Flight Log Report", rows: flightStatusRows.map((row) => [row.label, row.value]) },
        { key: "pre-inspection", category: "Logbook", title: "Pre-Inspection Report", rows: preRows.map((row) => [row.label, row.value]) },
        { key: "post-inspection", category: "Logbook", title: "Post-Inspection Report", rows: postRows.map((row) => [row.label, row.value]) },
        { key: "parts-requisition", category: "Inventory", title: "Parts Requisition Report", rows: reqRows.map((row) => [row.label, row.value]) },
      ];
    };

    const renderGroups = () => {
      const query = String(search?.value || "").trim().toLowerCase();
      const modules = reportModules().filter((module) => !query || [module.title, module.category, module.key].join(" ").toLowerCase().includes(query));
      groupTitle.textContent = query ? "Search Results" : "Grouped Analytics Modules";
      groupCopy.textContent = query ? "Showing matched report modules." : "Related tables and charts are grouped by domain for easier review.";
      const grouped = modules.reduce((acc, module) => {
        acc[module.category] ||= [];
        acc[module.category].push(module);
        return acc;
      }, {});
      groupsRoot.innerHTML = Object.entries(grouped).map(([category, items]) => `
      ...
      `).join("") || `<section class="card"><p class="muted">No report modules match the search.</p></section>`;
    };
    ...
    const render = () => {
      ...
      ].map(([label, value, view]) => `<button type="button" class="card report-kpi ${state.activeView === view ? "active" : ""}" data-report-view="${view === "modules" ? "dueSoon" : view}"><span class="muted">${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></button>`).join("");
      $("[data-top-damaged-base]", page).textContent = `Top: ${summary.topDamaged.label} (${summary.topDamaged.value})`;
      $("[data-top-repaired-base]", page).textContent = `Top: ${summary.topRepaired.label} (${summary.topRepaired.value})`;
      renderMiniChart($("[data-damage-chart]", page), summary.damageRows);
      renderMiniChart($("[data-repair-chart]", page), summary.repairRows);
      $$("[data-report-view]", page).forEach((button) => button.classList.toggle("active", button.dataset.reportView === state.activeView));
      renderTable();
      renderGroups();
    };
    ...
    const load = async () => {
      ...
      const entries = await Promise.all(Object.entries(endpoints).map(async ([key, url]) => {
        try {
          ...
          return [key, key === "baseAnalytics" ? payload.data || payload : normalizeRows(payload)];
        } catch (error) {
    ...
    const exportCsv = () => {
      ...
      reportModules().forEach((module) => module.rows.forEach(([label, value]) => rows.push([module.title, label, value])));
    ...
    const exportReportPrint = () => {
      ...
      win.document.write(`<!doctype html><html><head><title>AirMS Reports and Analytics</title><style>
        ...
        <table><thead><tr><th>Base Damage</th><th>Count</th><th>Base Repair</th><th>Count</th></tr></thead><tbody>
          ${Array.from({ length: Math.max(summary.damageRows.length, summary.repairRows.length, 1) }).map((_, index) => `<tr><td>${escapeHtml(summary.damageRows[index]?.label || "")}</td><td>${escapeHtml(summary.damageRows[index]?.value ?? "")}</td><td>${escapeHtml(summary.repairRows[index]?.label || "")}</td><td>${escapeHtml(summary.repairRows[index]?.value ?? "")}</td></tr>`).join("")}
        </tbody></table>
  ...
  const initPartsRequisitionPage = () => {
    ...
    const effectiveStatus = (record) => {
      ...
      return status === "Parts Requested" && record?.dateWarehouseReviewed ? "Availability Checked" : status;
    };
    ...
    const isManager = () => ["superadmin", "maintenance manager", "officer-in-charge"].includes(getUserRole());
    const isWarehouse = () => getUserRole() === "warehouse department";
    const canRequestParts = () => !["superadmin", "maintenance manager", "officer-in-charge", "warehouse department"].includes(getUserRole());
    const staffName = () => [getUser()?.firstName, getUser()?.lastName].filter(Boolean).join(" ") || getUser()?.username || "AirMS User";
    const dateText = (value) => {
      if (!value) return "";
      const date = new Date(value);
      return Number.isNaN(date.getTime()) ? String(value) : `${date.getMonth() + 1}/${String(date.getDate()).padStart(2, "0")}/${date.getFullYear()}`;
    };
    const normalizeRecord = (record = {}) => ({
      ...record,
      wrsNo: record.wrsNo || record.requestNumber || `WRS-${String(getId(record) || "").slice(-3)}`,
      status: normalizeReqStatus(record.status),
      dateRequested: dateText(record.dateRequested || record.createdAt),
      staff: {
        ...(record.staff || {}),
        employeeName: record.staff?.employeeName || record.staff?.requisitioner || record.requestedBy || "",
      },
      items: Array.isArray(record.items) && record.items.length ? record.items : [{
        _id: "item-1",
        ...
        stockStatus: record.stockStatus || record.status || "Parts Requested",
      }],
    });
    const totalQty = (record) => (record.items || []).reduce((sum, item) => sum + (Number(item.quantity) || 0), 0);
    const parseReqDate = (record) => {
      const date = new Date(record.dateRequested || record.createdAt || 0);
      return Number.isNaN(date.getTime()) ? 0 : date.getTime();
    };
    const nextWrsNo = () => {
      const highest = requisitions.reduce((max, item) => Math.max(max, Number(String(item.wrsNo || "").replace(/\D/g, "")) || 0), 0);
      return `WRS-${String(highest + 1).padStart(3, "0")}`;
    };
    const filtered = () => {
      const query = String(search.value || "").trim().toLowerCase();
      let rows = requisitions.filter((record) => {
        const haystack = [record.wrsNo, record.aircraft, effectiveStatus(record), record.staff?.employeeName].join(" ").toLowerCase();
        if (query && !haystack.includes(query)) return false;
        if (selectedStatus === "all") return true;
        if (selectedStatus === "for_review") return ["availability checked", "ordered"].includes(effectiveStatus(record).toLowerCase());
        if (selectedStatus === "completed") return ["delivered", "cancelled"].includes(effectiveStatus(record).toLowerCase());
        return statusBucket(record) === selectedStatus;
      });
      rows = [...rows].sort((a, b) => sort.value === "oldest" ? parseReqDate(a) - parseReqDate(b) : parseReqDate(b) - parseReqDate(a));
      return rows;
    };
    const statusFilters = () => {
      if (isManager()) return [
        ["all", "All", requisitions.length],
        ["for_review", "For Review", requisitions.filter((record) => ["availability checked", "ordered"].includes(effectiveStatus(record).toLowerCase())).length],
        ["closed", "Closed", requisitions.filter((record) => ["delivered", "cancelled", "approved"].includes(effectiveStatus(record).toLowerCase())).length],
      ];
      if (isWarehouse()) return [
        ["all", "All", requisitions.length],
        ["pending", "Pending", requisitions.filter((record) => !["delivered", "cancelled"].includes(effectiveStatus(record).toLowerCase())).length],
        ["completed", "Completed", requisitions.filter((record) => ["delivered", "cancelled"].includes(effectiveStatus(record).toLowerCase())).length],
      ];
      return [
        ["all", "All", requisitions.length],
        ["pending", "Pending", requisitions.filter((record) => statusBucket(record) === "pending").length],
        ["approved", "Approved", requisitions.filter((record) => statusBucket(record) === "approved").length],
        ["closed", "Closed", requisitions.filter((record) => statusBucket(record) === "closed").length],
      ];
    };
    const renderFilters = () => {
      filtersRoot.innerHTML = statusFilters().map(([key, title, count]) => `<button type="button" class="btn ${selectedStatus === key ? "" : "ghost"}" data-req-filter="${key}">${escapeHtml(title)} (${count})</button>`).join("");
    };
    const renderTable = () => {
      const rows = filtered();
      ...
      `).join("") : `<tr><td colspan="8" class="muted">No requisitions found.</td></tr>`;
    };
    ...
    const collectItems = () => $$("[data-req-item-row]", reqItems).map((row, index) => ({
      itemNo: index + 1,
    ...
    const loadAircraft = async () => {
      aircraftOptions = normalizeRows(await apiFetch("/api/parts-monitoring/aircraft-list").catch(() => []));
    ...
    const load = async () => {
      ...
      requisitions = normalizeRows(await apiFetch("/api/parts-requisition/get-all-requisition")).map(normalizeRecord);
      renderTable();
    };
    const itemStockStatus = (item, availableQty) => {
      const current = normalizeReqStatus(item.stockStatus);
      if (["Approved", "Delivered", "Cancelled"].includes(current)) return current;
      if (current === "To Be Ordered" || current === "Ordered") return availableQty >= Number(item.quantity || 0) ? "Ordered" : "To Be Ordered";
      return availableQty >= Number(item.quantity || 0) ? "In Stock" : "Out of Stock";
    };
    const currentAction = () => {
      const status = effectiveStatus(selectedRecord);
      const hasOut = (selectedRecord.items || []).some((item) => Number(item.availableQty || 0) < Number(item.quantity || 0));
      if (status === "Approved") return isWarehouse() ? ["Delivery", "Warehouse can mark this approved requisition as delivered.", "Mark Delivered", false] : ["Awaiting Delivery", "Approved requisition is waiting for warehouse delivery.", "Waiting", true];
      if (["Delivered", "Cancelled"].includes(status)) return ["Completed", "No further action is needed.", "Done", true];
      if (status === "Availability Checked") return isManager() ? [hasOut ? "Order Decision" : "Approval Decision", hasOut ? "Some items are short. Send to ordering." : "All requested quantities are available.", hasOut ? "Mark To Be Ordered" : "Approve", false] : ["Awaiting Maintenance Review", "Warehouse submitted availability.", "Waiting", true];
      if (status === "To Be Ordered") return isWarehouse() ? ["Confirm Restock", "Update available quantities, then mark as restocked.", "Mark as Restocked", false] : ["Awaiting Warehouse Restock", "Warehouse is confirming restocked quantities.", "Waiting", true];
      if (status === "Ordered") return isManager() ? ["Final Approval", "Warehouse confirmed restock.", "Approve", false] : ["Awaiting Approval", "Waiting for maintenance approval.", "Waiting", true];
      return isWarehouse() ? ["Stock Review", "Enter available quantities for all items.", "Submit Stock Review", false] : ["Awaiting Warehouse Review", "Warehouse is reviewing stock availability.", "Waiting", true];
    };
    const renderWrs = () => {
      ...
      wrsItems.innerHTML = (selectedRecord.items || []).map((item, index) => `
        <tr>
      ...
      const hasOut = (selectedRecord.items || []).some((item) => Number(item.availableQty || 0) < Number(item.quantity || 0));
      if (hasOut || selectedRecord.dateOrdered || ["To Be Ordered", "Ordered"].includes(status)) steps.push("To Be Ordered", "Ordered");
      steps.push("Approved", "Delivered");
      const currentIndex = steps.indexOf(status);
      wrsTimeline.innerHTML = steps.map((step, index) => `<li class="${index < currentIndex ? "done" : index === currentIndex ? "active" : ""}">${escapeHtml(step)}</li>`).join("");
    ...
    const updateSelected = async (updates) => {
      await apiFetch(`/api/parts-requisition/update-requisition/${encodeURIComponent(getId(selectedRecord))}`, {
        method: "POST",
        body: JSON.stringify({ ...updates, confirmAction: true }),
      });
      await load();
      selectedRecord = requisitions.find((record) => getId(record) === getId(selectedRecord)) || null;
      ...
      try {
        ...
        await apiFetch("/api/parts-requisition/create-requisition", {
          ...
          body: JSON.stringify({
            ...
            staff: { requisitioner: staffName(), employeeName: staffName(), approvedBy: "", receiver: "", notedBy: "", warehouseBy: "", deliveredBy: "" },
            items,
    ...
    tableBody?.addEventListener("click", (event) => {
      ...
      if (!button) return;
      selectedRecord = requisitions.find((record) => getId(record) === button.dataset.wrsOpen);
      renderWrs();
      wrsDialog.showModal();
    });
    ...
    $("[data-wrs-submit]")?.addEventListener("click", async () => {
      ...
      try {
        if (status === "Parts Requested") {
          await updateSelected({ status: "Parts Requested", dateWarehouseReviewed: new Date().toISOString(), warehouseBy: warehouse, items: selectedRecord.items });
        } else if (status === "Availability Checked") {
          const hasOut = selectedRecord.items.some((item) => Number(item.availableQty || 0) < Number(item.quantity || 0));
          await updateSelected(hasOut ? { status: "To Be Ordered" } : { status: "Approved", approvedBy: reviewer, approvedAt: new Date().toISOString() });
        } else if (status === "To Be Ordered") {
          await updateSelected({ status: "Ordered", dateOrdered: new Date().toISOString(), warehouseBy: warehouse, items: selectedRecord.items.map((item) => ({ ...item, stockStatus: itemStockStatus(item, Number(item.availableQty || 0)) })) });
        } else if (status === "Ordered") {
          await updateSelected({ status: "Approved", approvedBy: reviewer, approvedAt: new Date().toISOString() });
        } else if (status === "Approved") {
          await updateSelected({ status: "Delivered", dateDelivered: new Date().toISOString(), dateReceived: new Date().toISOString(), deliveredBy: warehouse, warehouseBy: warehouse, items: selectedRecord.items.map((item) => ({ ...item, stockStatus: "Delivered" })) });
        }
      } catch (error) {
    ...
    $$("[data-req-close]").forEach((button) => button.addEventListener("click", () => dialog.close()));
    $$("[data-wrs-close]").forEach((button) => button.addEventListener("click", () => wrsDialog.close()));
    if (isManager()) $("[data-req-new]")?.setAttribute("hidden", "hidden");
    Promise.all([loadAircraft(), load()]).catch((error) => { tableBody.innerHTML = `<tr><td colspan="8" class="muted">${escapeHtml(error.message)}</td></tr>`; });
  };
  ...
  const initMessages = () => {
    ...
    const fullName = (user = {}) => [user.firstName, user.lastName].filter(Boolean).join(" ").trim() || user.username || "User";
    const imageUrl = (image) => image ? (String(image).startsWith("http") ? image : image) : "";
    const initials = (name) => String(name || "U").split(/\s+/).map((part) => part.charAt(0)).join("").slice(0, 2).toUpperCase() || "U";
    ...
    const attachmentUrl = (url) => !url ? "" : String(url).startsWith("http") || String(url).startsWith("blob:") ? url : url;
    const attachmentLabel = (attachments = []) => {
      const first = attachments[0];
      if (!first) return "";
      const prefix = first.kind === "image" ? "Photo" : "File";
      return attachments.length > 1 ? `${prefix}: ${first.name} +${attachments.length - 1}` : `${prefix}: ${first.name}`;
    };
    const conversationTitle = (conversation) => conversation?.type === "group" ? conversation.group?.name || conversation.name || "Group chat" : fullName(conversation.user || conversation);
    const conversationSubtitle = (conversation) => conversation?.type === "group" ? `${conversation.group?.members?.length || 0} members` : conversation.user?.jobTitle || conversation.jobTitle || "User";
    const conversationId = (conversation) => String(conversation.id || entityId(conversation.group) || entityId(conversation.user) || entityId(conversation));
    ...
    const mergeConversationItems = () => {
      ...
      const items = conversations.map((conversation) => {
        if (conversation.type === "group") {
          return { ...conversation, id: conversationId(conversation), title: conversationTitle(conversation), subtitle: conversationSubtitle(conversation) };
        }
        const id = String(conversation.id || entityId(conversation.user));
        directIds.add(id);
        return { ...conversation, type: "direct", id, title: conversationTitle(conversation), subtitle: conversationSubtitle(conversation) };
      });
      users.forEach((user) => {
        const id = String(entityId(user));
        if (!id || directIds.has(id) || id === currentUserId()) return;
        items.push({ type: "direct", id, user, title: fullName(user), subtitle: user.jobTitle || "User", unreadCount: 0 });
      });
      const query = String(search?.value || "").trim().toLowerCase();
      return items
        .filter((item) => !query || [item.title, item.subtitle, item.user?.username, item.user?.email].filter(Boolean).join(" ").toLowerCase().includes(query))
    ...
    const renderConversations = () => {
      ...
      convRoot.innerHTML = "";
      items.forEach((item) => {
        const preview = item.lastMessage?.body || attachmentLabel(item.lastMessage?.attachments || []) || item.subtitle || "";
        const button = document.createElement("button");
        button.type = "button";
    ...
    const renderGroupMembers = () => {
      ...
      `).join("") : `<p class="muted">No members listed.</p>`;
    };

    const renderAttachments = (message) => (message.attachments || []).map((attachment) => {
      const url = attachmentUrl(attachment.url);
      const isImage = attachment.kind === "image" || String(attachment.mimeType || "").startsWith("image/");
      if (isImage && url) return `<a href="${escapeHtml(url)}" target="_blank" rel="noreferrer" class="message-attachment image"><img src="${escapeHtml(url)}" alt="${escapeHtml(attachment.name || "Attachment")}"></a>`;
    ...
    const loadThread = async (silent = false) => {
      ...
      if (!silent) threadRoot.innerHTML = `<p class="muted">Loading thread...</p>`;
      const rows = normalizeRows(await apiFetch(`/api/messages/${encodeURIComponent(activeConversation.id)}`));
      threadRoot.innerHTML = rows.length ? "" : `<p class="muted">No messages yet.</p>`;
      rows.forEach((message) => {
        const mine = String(message.from || message.sender || "") === currentUserId();
    ...
    const loadAll = async (silent = false) => {
      if (!silent) convRoot.innerHTML = `<p class="muted">Loading conversations...</p>`;
      ...
      groupMembers.innerHTML = users
        ...
        .map((user) => `<option value="${escapeHtml(entityId(user))}">${escapeHtml(fullName(user))} - ${escapeHtml(user.jobTitle || "User")}</option>`)
        .join("");
      renderConversations();
      if (activeConversation?.id) {
        const refreshed = mergeConversationItems().find((item) => item.type === activeConversation.type && item.id === activeConversation.id);
    ...
    const renderSelectedFiles = () => {
      attachmentsRoot.innerHTML = selectedFiles.length ? selectedFiles.map((file, index) => `
    ...
    sendForm?.addEventListener("submit", async (event) => {
      ...
      if (sendForm.recipientId.value) formData.append("recipientId", sendForm.recipientId.value);
      if (sendForm.conversationId.value) formData.append("conversationId", sendForm.conversationId.value);
      formData.append("body", body);
      selectedFiles.forEach((file) => formData.append("attachments", file));
      const nextHeaders = headers();
      delete nextHeaders["Content-Type"];
      try {
        const response = await fetch("/api/messages", { method: "POST", headers: nextHeaders, body: formData });
        const data = await response.json().catch(() => ({}));
    ...
    filesInput?.addEventListener("change", (event) => {
      selectedFiles = [...selectedFiles, ...Array.from(event.target.files || [])].slice(0, 5);
      renderSelectedFiles();
    });
    ...
    $("[data-pick-attachments]")?.addEventListener("click", () => filesInput?.click());
    search?.addEventListener("input", renderConversations);
    $("[data-message-back]")?.addEventListener("click", () => document.body.classList.remove("message-chat-open"));
    $("[data-open-group-modal]")?.addEventListener("click", () => {
      groupStatus.textContent = "";
      groupDialog.showModal();
    });
    $$("[data-close-group]").forEach((button) => button.addEventListener("click", () => groupDialog.close()));
    groupForm?.addEventListener("submit", async (event) => {
      event.preventDefault();
      const memberIds = Array.from(groupMembers.selectedOptions).map((option) => option.value);
      try {
        const payload = await apiFetch("/api/messages/groups", { method: "POST", body: JSON.stringify({ name: groupForm.name.value, memberIds }) });
        groupDialog.close();
        groupForm.reset();
        await loadAll(true);
        ...
        if (group?._id || group?.id) {
          ...
          const item = mergeConversationItems().find((conversation) => conversation.type === "group" && conversation.id === id);
    ...
    $$("[data-close-members]").forEach((button) => button.addEventListener("click", () => membersDialog.close()));
    if (window.EventSource) {
      const stream = new EventSource("/api/events/stream");
      stream.addEventListener("airms:data-changed", (event) => {
        try {
          const payload = JSON.parse(event.data || "{}");
          if (String(payload.url || "").startsWith("/api/messages")) loadAll(true);
        } catch {
          loadAll(true);
        }
      });
